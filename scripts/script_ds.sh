...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -v 1.2 -c '{"Args":["c","asdf"]}' -P "AND('Org1MSP.member','Org1MSP.member')" >&log.txt
	else
		peer chaincode instantiate -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n myccds -v 1.2 -c '{"Args":["c","asdf"]}' -P "AND('Org1MSP.member','Org1MSP.member')" >&log.txt
	fi
	res=$?
	cat log.txt
