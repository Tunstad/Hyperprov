	...
	if [ $ORG -eq 1 ]; then
		...
		if [ $1 -eq 0 -o $1 -eq 1 ] ; then
			...
			if [ $1 -eq 0 ]; then
				...
				CORE_PEER_ADDRESS=node1.ptunstad.no:7051
			else
				...
				CORE_PEER_ADDRESS=node2.ptunstad.no:7051
			fi
		else
			if [ $1 -eq 2 ]; then
				...
				CORE_PEER_ADDRESS=node3.ptunstad.no:7051
			else
				...
				CORE_PEER_ADDRESS=node0.ptunstad.no:7051
			fi
		fi
	else
...
# 		CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org3.ptunstad.no/peers/peer0.org3.ptunstad.no/tls/ca.crt
# 		CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/org3.ptunstad.no/users/Admin@org3.ptunstad.no/msp
# 		if [ $PEER -eq 0 ]; then
# 			CORE_PEER_ADDRESS=peer0.org3.ptunstad.no:7051
# 		else
# 			CORE_PEER_ADDRESS=peer1.org3.ptunstad.no:7051
# 		fi
# 	else
# 		echo "================== ERROR !!! ORG Unknown =================="
# 	fi
...
installChaincode () {
	...
	VERSION=${3:-1.2}
        set -x
	peer chaincode install -n myccds -v ${VERSION} -l ${LANGUAGE} -p ${CC_SRC_PATH} >&log.txt
	res=$?
        set +x
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                set -x
		peer chaincode instantiate -o orderer.ptunstad.no:7050 -C $CHANNEL_NAME -n myccds -l ${LANGUAGE} -v ${VERSION} -c '{"Args":["c","51","file://mnt/hlfshared", "cfile"]}' -P "OR('Org1MSP.member','Org1MSP.member')" >&log.txt ## '{"Args":["c","asdf"]}'
		res=$?
                set +x
	else
                set -x
...
upgradeChaincode () {
    ...
    set -x
    peer chaincode upgrade -o orderer.ptunstad.no:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n myccds -v 2.0 -c '{"Args":["init","a","90","b","210"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')"
    res=$?
	set +x
    cat log.txt
...
chaincodeQuery () {
  ...
  do
    ...
    set -x
    peer chaincode query -C $CHANNEL_NAME -n myccds -c '{"Args":["checkhash","c"]}' >&log.txt
    res=$?
    set +x
